...
{
	return flag_present(flags, TEE_SHM_MAPPED | TEE_SHM_TEMP);
}
...
static char *_uuid_to_str(const struct teec_uuid *uuid)
{
	static char uuid_str[_UUID_STR_SIZE];

	if (uuid) {
		uint8_t *seq_mode = seq_mode;
...
static int to_memref_type(int flags)
{
	...
	if (flag_present(flags, TEEC_MEM_INPUT))
		return TEEC_MEMREF_TEMP_INPUT;

	if (flag_present(flags, TEEC_MEM_OUTPUT))
			 ...
			 struct tee_cmd *cmd)
{
	...
	for (idx = 0; idx < TEEC_CONFIG_PAYLOAD_REF_COUNT; ++idx) {
		...
		case TEEC_MEMREF_WHOLE:
			...
			if (IS_ERR_OR_NULL(param->params[idx].shm)) {
				if (0) {
					dev_err(_DEV_TEE, "< can't get shm\n");
					goto out;
				}
